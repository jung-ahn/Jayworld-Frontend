...
import { faUser,faCameraRetro,faSignOutAlt } from '@fortawesome/free-solid-svg-icons';
...
export class NavbarComponent implements OnInit, OnDestroy {
  ...
  private subscriptions: Subscription[] = [];  //make it in array so we can add and destroy.
  user: User;
  searchedUser: User[];
  host: string;
  userHost: string;
  ...
  ngOnInit() {
    this.loadingService.isLoading.next(true);  //turn on loading component at the first side.
  ...
  //       (response: any) => {
  //         this.location = response.results[3]; //4th index of google api has the info of location
  //       },
  //       error => {
  //         console.log(error);
  //       }
